...
{
    symtab_enter_scope();
    scope_check_varDecls(prog.block->var_decls);
    // need to update stmt's AST with id_use structs
    prog.block->stmts = scope_check_stmts(prog.block->stmts);
    symtab_leave_scope();
    return prog;
}
...
void scope_check_idents(ident_list_t ids,
			id_kind t)
{
...
// (if not, then produce an error)
// Return the modified AST with id_use pointers
stmt_list_t scope_check_stmts(stmt_list_t stmts)
{
...
{
    stmt.condition = scope_check_expr(stmt.condition);
